CREATE OR ALTER VIEW dbo.v_kpi_monthly_neighborhood_2024 AS
WITH base AS (
  SELECT
    open_month,
    neighborhood,
    ...
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY resolution_hours)
      OVER (PARTITION BY open_month, neighborhood) AS median_resolution_hours
...
FROM base
GROUP BY open_month, neighborhood;
GO